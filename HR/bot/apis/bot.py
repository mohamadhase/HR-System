...
def message(payload):
    ...
    if payload['event']['text'] == 'help':
        ...
        client.chat_postMessage(
            channel='@'+payload['event']['user'], blocks=blocks, token=getenv('SLACK_APP_TOKEN'))
...
class AdminEmployees(Resource):
    ...
    def post(self):
        ...
        for employee in employees:
            ...
            team_id_section = Bot.create_section(
                f'Team ID: {employee["TeamID"]}')
            phone_section = Bot.create_section(f'Phone: {employee["Phone"]}')
            email_section = Bot.create_section(f'Email: {employee["Email"]}')
            hour_price = Bot.create_section(
                f'Hour Price: {employee["HourPrice"]}')
...
class AdminTeams(Resource):
    ...
    def post(self):
        ...
        for index, teams in enumerate(teams):
            ...
            description_section = Bot.create_section(
                f'Team Description : {teams["Description"]}')
...
class AdminUser(Resource):
    ...
    def post(self):
        ...
        blocks.append(Bot.create_section(
            f'Hour Price: {user_info["HourPrice"]}'))
        blocks.append(Bot.create_divider())
...
class AddAttendance(Resource):
    def post(self):
        ...
        date_dict = {'Year': today_date.year, 'Month': today_date.month, 'Day': today_date.day,
                     'NumberOfHours': number_of_hours, 'Date': today_date.strftime('%Y-%m-%d')}
        # check if the user attendance is already exists
        if Employee.is_attend(org_id, user_id, date_dict)[0]:
            blocks.append(Bot.create_section(
                'User attendance is already exists'))
            blocks.append(Bot.create_divider())
...
class GetAttend(Resource):
    def post(self):
        ...
        blocks.append(Bot.create_section(
            f'NumberOfHours: {attendance_info["NumberOfHours"]}'))
        blocks.append(Bot.create_divider())
...
class AttendanceReport(Resource):
    def post(self):
        ...
        attends = [attend for attend in attends if attend['Month']
                   == month and attend['Year'] == year]
        ...
        blocks.append(Bot.create_section(
            f'Attendance Report for {month}/{year}'))
        blocks.append(Bot.create_section(
            f'This Month consists of {daysInMonth} days'))
        blocks.append(Bot.create_section(
            f'You have attended {len(attends)} days'))
        for attend in attends:
            blocks.append(Bot.create_section(f'Date: {attend["Date"]}'))
            blocks.append(Bot.create_section(
                f'NumberOfHours: {attend["NumberOfHours"]}'))
            blocks.append(Bot.create_divider())
        blocks.append(Bot.create_section(
            f'Total Hours: {sum([attend["NumberOfHours"] for attend in attends])}'))
        blocks.append(Bot.create_section(
            f'Your Attendance Percentage: {(len(attends)/daysInMonth)*100}%'))
        blocks.append(Bot.create_divider())
        return {'blocks': blocks}
...
class Salary(Resource):
    def post(self):
        ...
        number_of_hours = int(
            sum([attend['NumberOfHours'] for attend in attends]))
        # get the total salary of the user
        salary = number_of_hours * hour_price
        blocks.append(Bot.create_section(
            f'Your Salary for {month}/{year} is {salary}'))
        blocks.append(Bot.create_section(
            f'You have attended {number_of_hours} hours'))
...
class AdminAttendance(Resource):
    ...
    def post(self):
        ...
        if user_id:
            employees = [
                employee for employee in employees if employee['ID'] == user_id]
        # filter the attendant employees
        employees = [employee for employee in employees if Employee.is_attend(
            org_id, employee['ID'], date)[0]]

        # create the attendance report for slack
        if user_id:
            if len(employees) == 0:
                blocks.append(Bot.create_section(
                    'No The Employee is not present'))
                blocks.append(Bot.create_divider())
                return {'blocks': blocks}
            else:
                blocks.append(Bot.create_section(
                    f'Attendance Report for {date["Date"]}'))
        ...
        blocks.append(Bot.create_section(
            f'Attendance Report for {date["Date"]}'))
        blocks.append(Bot.create_section(
            f'Total Employees attended: {len(employees)}'))
        for employee in employees:
